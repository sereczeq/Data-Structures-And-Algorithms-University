...
class Document implements IWithName
{
	...
	private static final Pattern linkPattern = Pattern.compile("(link=[\\w]+(\\([0-9]+\\))*)",
			Pattern.CASE_INSENSITIVE);
	private static final Pattern linkWithWeightPattern = Pattern.compile("(link=[\\w]+(\\([0-9]+\\))+)",
			Pattern.CASE_INSENSITIVE);
	...
	static Link createLink(String link)
	{
		...
		if (linkMatcher.matches())
		{
			...
			if (linkWithWeightMatcher.matches())
			{
				...
				{
					String rawWeight = parenthesesMatcher.group(1).replace("(", "").replace(")", "");
